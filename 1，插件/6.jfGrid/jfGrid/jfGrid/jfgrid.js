    ...
    $.extend($.jfGrid, {
        ...
        renderHead: function ($self) {// 渲染表格头部
            ...
            for (var i = 0; i < len; i++) {
                ...
                cell.left = left;
                if (!!cell.display) {
                    $cell.css({ 'width': ((cell.width || 100) + 'px'), 'text-align': (cell.align || 'left'), 'left': (left + 'px'),'display':cell.display });
                } else {
        ...
        getCellHtml: function (node, row, dfop, $dcell, rownum) {
            ...
            if (!!node.editType) {
                ...
                }if(!!node.formatterClick){   //扩展操作事件
                    node.formatterClick(value, row, dfop, $dcell);
                }
                else {
                    res = value;
